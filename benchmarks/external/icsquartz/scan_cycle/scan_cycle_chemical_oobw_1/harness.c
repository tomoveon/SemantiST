...
#define SEMANTIST_INSTANCE_SIZE 1512
#define SEMANTIST_INPUT_SIZE    672
...
int LLVMFuzzerTestOneInput(const uint8_t *Data, size_t Size) {
    ...
    if (off < Size) {
        size_t n = 328;
        if (n > Size - off) n = Size - off;
        memcpy(semantist_fuzzer_instance + 336, Data + off, n);
        off += n;
    }
    if (off < Size) {
        size_t n = 8;
        if (n > Size - off) n = Size - off;
        memcpy(semantist_fuzzer_instance + 664, Data + off, n);
        off += n;
    }
