...
#define SEMANTIST_INSTANCE_SIZE 400
#define SEMANTIST_INPUT_SIZE    184
...
int LLVMFuzzerTestOneInput(const uint8_t *Data, size_t Size) {
    ...
    if (off < Size) {
        size_t n = 4;
        if (n > Size - off) n = Size - off;
        memcpy(semantist_fuzzer_instance + 168, Data + off, n);
        off += n;
    }
    if (off < Size) {
        size_t n = 1;
        if (n > Size - off) n = Size - off;
        memcpy(semantist_fuzzer_instance + 172, Data + off, n);
        off += n;
    }
    if (off < Size) {
        size_t n = 8;
        if (n > Size - off) n = Size - off;
        memcpy(semantist_fuzzer_instance + 176, Data + off, n);
        off += n;
    }
