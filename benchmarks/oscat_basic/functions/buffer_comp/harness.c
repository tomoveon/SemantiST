...
#define SEMANTIST_INSTANCE_SIZE 32
#define SEMANTIST_INPUT_SIZE    28
...
static unsigned char semantist_ptr_buf_0[4096];
static unsigned char semantist_ptr_buf_2[4096];
...
int LLVMFuzzerTestOneInput(const uint8_t *Data, size_t Size) {
    ...
    if (off < Size) {
        ...
        memcpy(semantist_ptr_buf_0, Data + off, n);
        memset(semantist_ptr_buf_0 + n, 0, 4096 - n);
        off += n;
    } else {
        memset(semantist_ptr_buf_0, 0, 4096);
    }
    { void *semantist_ptr = semantist_ptr_buf_0; memcpy(semantist_fuzzer_instance + 0, &semantist_ptr, sizeof(semantist_ptr)); }
    if (off < Size) {
        size_t n = 2;
        if (n > Size - off) n = Size - off;
    ...
    if (off < Size) {
        ...
        memset(semantist_ptr_buf_2 + n, 0, 4096 - n);
        off += n;
    } else {
        memset(semantist_ptr_buf_2, 0, 4096);
    }
    { void *semantist_ptr = semantist_ptr_buf_2; memcpy(semantist_fuzzer_instance + 16, &semantist_ptr, sizeof(semantist_ptr)); }
    if (off < Size) {
        size_t n = 2;
        if (n > Size - off) n = Size - off;
    ...
    if (off < Size) {
        size_t n = 2;
        if (n > Size - off) n = Size - off;
        memcpy(semantist_fuzzer_instance + 26, Data + off, n);
        off += n;
    }
