...
#define SEMANTIST_INSTANCE_SIZE 272
#define SEMANTIST_INPUT_SIZE    266
...
int LLVMFuzzerTestOneInput(const uint8_t *Data, size_t Size) {
    ...
    if (off < Size) {
        size_t n = 2;
        if (n > Size - off) n = Size - off;
        memcpy(semantist_fuzzer_instance + 252, Data + off, n);
        off += n;
    }
    if (off < Size) {
        size_t n = 4096;
    ...
    { void *semantist_ptr = semantist_ptr_buf_2; memcpy(semantist_fuzzer_instance + 256, &semantist_ptr, sizeof(semantist_ptr)); }
    if (off < Size) {
        size_t n = 2;
        if (n > Size - off) n = Size - off;
        memcpy(semantist_fuzzer_instance + 264, Data + off, n);
        off += n;
    }
