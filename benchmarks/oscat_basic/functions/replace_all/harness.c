...
#define SEMANTIST_INSTANCE_SIZE 1004
#define SEMANTIST_INPUT_SIZE    753
...
int LLVMFuzzerTestOneInput(const uint8_t *Data, size_t Size) {
    ...
    if (off < Size) {
        size_t n = 251;
        if (n > Size - off) n = Size - off;
        memcpy(semantist_fuzzer_instance + 251, Data + off, n);
        off += n;
    }
    if (off < Size) {
        size_t n = 251;
        if (n > Size - off) n = Size - off;
        memcpy(semantist_fuzzer_instance + 502, Data + off, n);
        off += n;
    }
